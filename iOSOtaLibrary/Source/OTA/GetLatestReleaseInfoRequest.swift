...
extension HTTPRequest {
    ...
    static func getLatestReleaseInfo(token: DeviceInfoToken, key: ProjectKey) -> HTTPRequest? {
        let parameters: [String: String] = [
            "hardware_version": token.percentEncodedString(\.hardwareVersion),
            "software_type": token.percentEncodedString(\.softwareType),
            "current_version": token.percentEncodedString(\.currentVersion),
            "device_serial": token.percentEncodedString(\.deviceSerialNumber)
        ]
