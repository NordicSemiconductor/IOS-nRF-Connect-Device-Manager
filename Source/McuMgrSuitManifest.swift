...
public class McuMgrSuitManifest: CBORMappable {
...
public class McuMgrSuitCommonStructure: CBORMappable {
    ...
    public required init(cbor: CBOR?) throws {
        ...
        if case let CBOR.byteString(sharedSequence)? = decodedCbor[0x04],
           ...
           let singleSequence = try? McuMgrSuitSharedSequence(cbor: decodedCbor) {
            self.sharedSequences = [singleSequence]
        } else if case let CBOR.array(sharedSequences)? = decodedCbor[0x04] {
            self.sharedSequences = sharedSequences.compactMap({
                try? McuMgrSuitSharedSequence(cbor: $0)
            })
        }
    }
}
...
public class McuMgrSuitSharedSequence: CBORMappable {
    
    public var conditionVendorIdentifier: UInt64?
    
    public var conditionClassIdentiifer: UInt64?
    
    public var directiveSetComponentIndex: UInt64?
    ...
    public required init(cbor: CBOR?) throws {
        ...
        if case let CBOR.unsignedInt(classIdentifier)? = cbor?[2] {
            self.conditionClassIdentiifer = classIdentifier
        }
        if case let CBOR.unsignedInt(componentIndex)? = cbor?[12] {
            self.directiveSetComponentIndex = componentIndex
        }
        try super.init(cbor: cbor)
    }
}
